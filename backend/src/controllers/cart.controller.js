...
export const addToCart = async (req, res) => {
    try {
        ...
        const variantId = req.params.variantId || req.params.varientId;
        const { quantity = 1 } = req.body;
        const numQuantity = Number(quantity) || 1;

        console.log("📥 [Add To Cart API] Request params:", { productId, variantId }, "Body:", req.body);

        // 1. Fetch product
        const product = await productModel.findById(productId);
        console.log("📦 [Add To Cart API] Fetched Product from DB:", product ? { id: product._id, title: product.title, variantsCount: product.variants?.length, variants: product.variants } : null);
        ...
        // 2. Resolve target variant from product.variants (or fallback to product.varients)
        const variantsList = product.variants || product.varients || [];
        let variantObj = variantsList.find(v => v._id?.toString() === variantId);
        ...
        console.log("🎯 [Add To Cart API] Resolved Target Variant:", variantObj ? { id: variantObj._id, title: variantObj.title, price: variantObj.price } : "No variant match found, falling back to product ID");
        ...
        if (isProductAlreadyInCart) {
            ...
            console.log("💾 [Add To Cart API] Cart updated in DB:", JSON.stringify(updatedCart, null, 2));
        ...
        await cart.save();
        const updatedCart = await cartModel.findOne({ user: req.user._id }).populate("items.product");
        console.log("💾 [Add To Cart API] New Item Added to Cart in DB:", JSON.stringify(updatedCart, null, 2));
